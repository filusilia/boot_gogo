...
import com.alice.boot.security.JwtAuthenticationFilter;
import com.alice.boot.security.JwtAuthenticationProvider;
import com.alice.boot.security.UserDetailsServiceImpl;
 ...
 * <p>
 * 4个注解可用：
 * <li>@PreAuthorize在方法调用之前, 基于表达式的计算结果来限制对方法的访问</li>
 * <li>@PostAuthorize允许方法调用, 但是如果表达式计算结果为false, 将抛出一个安全性异常</li>
 * <li>@PostFilter允许方法调用, 但必须按照表达式来过滤方法的结果</li>
 * <li>@PreFilter允许方法调用, 但必须在进入方法之前过滤输入值</li>
 * </p>
 * <p>
...
//                out.close();
            ...
            resp.sendError(HttpServletResponse.SC_UNAUTHORIZED, exception == null ? "Unauthorized" : exception.getMessage());
